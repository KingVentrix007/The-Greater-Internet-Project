...
def init_connection(username:str,password:str,user_url=None):
    # Step 1, get cert+rsa pub key
    if(user_url != None):
        server_cert = handle_cert.request_certificate(server_url=f"{user_url}/httpe-init")
    else:
    ...
    if(user_url == None):
        print("Assuming localhost server on port 8000")
        url = "http://localhost:8000/client-login"
    else:
        url = f"{user_url}/client-login"
    print("Here now")
...
import os
import time
def send_request_post(urequest=None,path=None,user_url=None):
    if(urequest==None):
        request = {"dummy":None}
    else:
        request = urequest
    if(user_url == None):
        print("Assuming localhost on port 8000")
